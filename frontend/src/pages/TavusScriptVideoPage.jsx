...
export default function TavusScriptVideoPage() {
  ...
  useEffect(() => {
    ...
    const pull = async () => {
      try {
        const res = await api.getTavusScriptVideoJob(jobId);
  ...
  const onCreate = async () => {
    ...
    setLoading(true);
    try {
      const res = await api.createTavusScriptVideoJob(scriptText.trim());
  ...
  return (
    <div className="glass rounded-xl p-6 space-y-4 max-w-4xl mx-auto" dir="rtl">
      <h1 className="text-xl font-semibold flex items-center gap-2">
        ...
        Tavus Script To Video
      </h1>
      <p className="text-sm text-midnight-400">
        עמוד נפרד: מזינים תסריט ושולחים לרינדור ב-Tavus לקבלת קובץ וידאו.
      </p>
      ...
      >
        <Sparkles size={18} />
        {loading ? 'שולח ל-Tavus…' : 'צור סרטון'}
      </button>
